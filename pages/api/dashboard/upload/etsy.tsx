...
export default async function handler(
          ...
          await fetch(
            ...
            .then(async (data) => {
              ...
              const variantImages = Array(variantTags.length).fill(
                Xss(
                  (
                    product.MainImage.url_570xN ||
                    product.MainImage.url_fullxfull
                  ).trim()
                )
              );
                ...
                .then(async (data) => {
                  data.results.products.forEach(
                    (product: {
                      offerings: Array<{
                        price: {
                          before_conversion: { currency_formatted_raw: string };
  ...
  if (error) {
    SumoLogic.log({
      ...
      message: `Failed to fetch products from Etsy: ${error.message}`,
      params: { body: reqBody },
    });
